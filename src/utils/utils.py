...
from .datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, ImageFolder_custom
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4, local_view=False):
    ...
    if partition == "homo":
        ...
        if num == 10:
            ...
            a = np.random.randint(low=0, high=K, size=num)
            remain = np.delete(np.arange(K), a)
            print(a)
            contain.append(copy.deepcopy(a.tolist()))
            for el in a:
                times[el]+=1
                
            for i in range(n_parties-1):
                x = np.random.randint(low=K//2, high=K)
                y = np.random.randint(low=0, high=(K//4)+1)
                ...
                labels = np.random.choice(a, size=s, replace=False).tolist()
                print(labels)
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, dataidxs_test=None):
    if dataset in ('mnist', 'femnist', 'fmnist', 'cifar10', 'cifar100',
                   'svhn', 'tinyimagenet', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY'):
        if dataset == 'mnist':
